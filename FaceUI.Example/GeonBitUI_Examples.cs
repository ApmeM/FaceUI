...
//-----------------------------------------------------------------------------
// This program show FaceUI examples and usage.
//
// FaceUI is an export of the UI system used for GeonBit (an open source 
// game engine in MonoGame) and is free to use under the MIT license.
//
// To learn more about FaceUI, you can visit the git repo:
// https://github.com/RonenNess/FaceUI
//
...
namespace FaceUI.Example
{
    /// <summary>
    /// FaceUI.Example is just an example code. Everything here is not a part of the FaceUI framework, but merely an example of how to use it.
    /// </summary>
    ...
    public class GeonBitUI_Examples : Game
    {
        ...
        protected void InitExamplesAndUI()
        {
            ...
            showGitButton.OnClick = (Entity btn) => { System.Diagnostics.Process.Start("https://github.com/RonenNess/FaceUI"); };
...
FaceUI is the UI system of the GeonBit project.
...
");
                ...
                // example: features list
                {
                    ...
                    panel.AddChild(new Paragraph(@"FaceUI implements the following widgets:
...
"));
                ...
                // example: basic concepts
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph(@"Panels are the basic containers of FaceUI. They are like window forms.
...
The most common anchors are 'Auto' and 'AutoInline', which will place entities one after another automatically.",
                ...
                // example: buttons
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with 3 button skins:"));
                ...
                // example: panels
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with 4 alternative panel skins:"));
                    int panelHeight = 80;
                    {
...
Here's a button, to test clicking while scrolled:"));
                ...
                // example: icons
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with some built-in icons:"));
...
Maybe something interesting in tab3?"));
                ...
                // example: messages
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with a utility to create simple message boxes:"));
                    ...
                    // button to create simple message box
                    {
                        ...
                        {
                            Utils.MessageBox.BuildMessageBox("Hello World!", "This is a simple message box. It doesn't say much, really.").Show();
                        };
                        panel.AddChild(btn);
                    }
                    ...
                    panel.AddChild(new Paragraph("Or you can create custom message and buttons:"));
                    {
                        ...
                        btn.OnClick += (Entities.Entity entity) =>
                        {
                            Utils.MessageBox.BuildMessageBox("Custom Message!", "In this message there are two custom buttons.\n\nYou can set different actions per button. For example, click on 'Surprise' and see what happens!", new Utils.MessageBox.MsgBoxOption[] {
                                new Utils.MessageBox.MsgBoxOption("Close", () => { return true; }),
                                new Utils.MessageBox.MsgBoxOption("Surprise", () => { Utils.MessageBox.BuildMessageBox("Files Removed Successfully", "Win32 was successfully removed from this computer. Please restart to complete OS destruction.\n\n(Just kidding!)").Show(); return true; })
                                }).Show();
                        };
                        panel.AddChild(btn);
                    }
                    ...
                    panel.AddChild(new Paragraph("And you can also add extra entities to the message box:"));
                    {
                        ...
                        btn.OnClick += (Entity entity) =>
                        {
                            ...
                            Utils.MessageBox.BuildMessageBox("Message With Extra!", "In this message box we attached an extra entity from outside (a simple text input).\n\nPretty neat, huh?", new Utils.MessageBox.MsgBoxOption[] {
                                new Utils.MessageBox.MsgBoxOption("Close", () => { return true; }),
                                }, new Entity[] { textInput }).Show();
                        };
                        panel.AddChild(btn);
                    }
                }
                ...
                // example: forms
                {
                    ...
                    btn.OnClick += (Entity ent) =>
                    {
                        ...
                        var msgBox = Utils.MessageBox.BuildMessageBox(
                            "Example Form",
                            "",
                            "Close Form And Show Values",
                            extraEntities: new Entity[] { newForm.FormPanel });
                        msgBox.OnDone = (b) =>
                        {
                            Utils.MessageBox.BuildMessageBox(
                                "Form Values",
                                string.Format(
                                    "Text Field: '{5}{0}{6}'\r\n" + "Slider: '{5}{1}{6}'\r\n"
                                                                  + "Radio Buttons: '{5}{2}{6}'\r\n"
                ...
                // example: file menu
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with a utility to generate a classic file menu:"));

                    var layout = new Utils.SimpleFileMenu.MenuLayout();
                    layout.AddMenu("File", 260);
                    layout.AddItemToMenu("File", "New", () => { Utils.MessageBox.BuildMessageBox("Something New!", "Lets make something new.").Show(); });
                    layout.AddItemToMenu("File", "Save", () => { Utils.MessageBox.BuildMessageBox("Something Saved!", "Your thing was saved successfully.").Show(); });
                    layout.AddItemToMenu("File", "Load", () => { Utils.MessageBox.BuildMessageBox("Something Loaded!", "Your thing was loaded successfully.").Show(); });
                    layout.AddItemToMenu("File", "Exit", () => { Utils.MessageBox.BuildMessageBox("Not Yet", "We still have much to see.").Show(); });
                    ...
                    panel.AddChild(new Paragraph("Usually this menu should cover the top of the screen and not be inside another panel. Note that like most entities in FaceUI, you can also set its skin:"));
                ...
                // example: Cursors
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph("FaceUI comes with 3 basic cursor types:"));
...
Click on 'Next' to see the character creation demo."));
                ...
                // example: epilogue
                {
                    ...
                    panel.AddChild(new HorizontalLine());
                    panel.AddChild(new Paragraph(@"That's it for now! There is still much to learn about FaceUI, but these examples were enough to get you going.

To learn more, please visit the git repo, read the docs, or go through some source code.

If you liked FaceUI feel free to star the repo on GitHub. :)"));
                }
